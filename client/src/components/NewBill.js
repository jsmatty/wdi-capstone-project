...
export default class NewBill extends Component {
    ...
    _addbill = async (e) => {
        ...
        const res = await axios.post('/api/household/:household_id/bills', payload)
  ...
  render() {
    return (
      <div>
          {this.state.redirect? 
            <Redirect to={`/household/:household_id/bills/:id`}/>
            :
        ...
        <div>    
         ...
         <div>
           ...
           <input onChange={this._handleChange} type="text" name="household_address" value={this.state.household_address} />
            ...
            <button><Link to="/household/:household_id/bills/:id">Add New bill</Link></button>
        </form>
          }
